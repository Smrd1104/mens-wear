...
import { getColorNameFromHex } from '../utils/colors'; // ✅ your color utility
...
const Orders = ({ token }) => {
  ...
  // ✅ Extract size and color separately
  const extractSizeAndColor = (size = '', hexColor = '') => {
    return {
      size,
      ...
      if (response.data.success) {
        const processedOrders = response.data.orders.map(order => ({
          ...
          items: order.items.map(item => ({
            ...item,
            variantInfo: extractSizeAndColor(item.size, item.color)
          }))
        }));
                              ...
                              {item.variantInfo.color && (
                                <div className="flex items-center gap-1 text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded">
                                  ...
                                  <div
                                    ...
                                    title={item.variantInfo.hexColor}
                                  />
                                </div>
                              )}
                            </div>
